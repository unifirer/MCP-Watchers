...
- Stale Memory: Always fix stale memory when discovered. This applies to all memory storage MCPs, including AutoMem and Graphiti via MCP. In Jcode, use the native memory tool built into Jcode. Group memory by code repository when relevant.

- Memory Storage: When storing memory, always write to all available memory backends — AutoMem, Graphiti MCP. Group entries by code repository where relevant.
