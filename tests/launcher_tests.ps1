...
try {
    ...
    # --- mcpw-tao: cut the launcher's combined-view FALLBACK out of this copy ---
    # The fallback (`if (-not $wtOk) { ... }`, launcher line 4629, just above the
    # controller loop) tails the four watcher logs forever. It is the interactive
    # Ctrl+C path, and T8 runs the block under Invoke-Expression, so whenever wt
    # fails to build the grid the suite never returns -- the 300 s budget in
    # test_launcher.py is unreachable and T9+ never runs at all (observed: killed
    # at 21 m 22 s). T8's assertions (WT window, four tailer files, live watcher
    # processes) do not need the tail, so swap it for a bounded stub. A failed wt
    # build then surfaces as an honest T8 assertion instead of a hang.
    $fbIdx = $paneBlock.IndexOf('if (-not $wtOk) {')
    if ($fbIdx -ge 0) {
        $paneBlock = $paneBlock.Substring(0, $fbIdx) + @'
if (-not $wtOk) {
    # Replaced for T8: the real fallback is an unbounded combined-view tail.
    Write-Host "  [T8] wt grid build failed (wtOk=$false) - combined-view tail disabled for this run."
}
'@
    }
