# tests/mcpw_rkg3_launcher_bootstrap.tests.ps1
# Pester 6 idiom (Should -Be / Should -Match). Bead mcpw-rkg.3 - wire the
# committed bootstrap module (Modules/watcher_mcp_bootstrap.ps1, bead
# mcpw-rkg.2) into the ###1 launcher.
#
# RUN IT WITH THE SUITE RUNNER:
#     python dev_tools/run_pester_suite.py tests/mcpw_rkg3_launcher_bootstrap.tests.ps1
...
#   5. A bootstrap failure cannot abort the launch: guarded by Get-Command,
...
Describe 'mcpw-rkg.3: the launcher bootstraps the six MCPs before spawning watchers' {

    It 'dot-sources the bootstrap module with the guarded Test-Path pattern' {
        ...
        $text | Should -Match '\$watcherMcpBootstrapModule = Join-Path \$scriptDir ''Modules\\watcher_mcp_bootstrap\.ps1'''
        $text | Should -Match 'if \(Test-Path -LiteralPath \$watcherMcpBootstrapModule\) \{ \. \$watcherMcpBootstrapModule \}'
        ...
        ([regex]::Matches($text, '\. \$watcherMcpBootstrapModule')).Count |
    ...
    It 'loads the module AFTER $watchersWorkspaceRoot is resolved' {
        ...
        $loadAt = $text.IndexOf('$watcherMcpBootstrapModule = Join-Path')
    ...
    It 'CALLS Invoke-McpBootstrapForRepo on the caller repository, before the first watcher spawn' {
        $launcher = Join-Path (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path '###1.watchers_for_memtrace_grepai_graphenium_graphify-rs_repowise.ps1'
        $text = Get-Content -LiteralPath $launcher -Raw

        $text | Should -Match 'Invoke-McpBootstrapForRepo -Path \$watchersWorkspaceRoot'
        ...
        $callAt = $text.IndexOf('Invoke-McpBootstrapForRepo -Path $watchersWorkspaceRoot')
    ...
    It 'is repo-agnostic: no absolute path anywhere in the bootstrap block' {
        $launcher = Join-Path (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path '###1.watchers_for_memtrace_grepai_graphenium_graphify-rs_repowise.ps1'
        $text = Get-Content -LiteralPath $launcher -Raw

        $start = $text.IndexOf('# --- MCP bootstrap, BEFORE the first watcher spawns')
        $end = $text.IndexOf("`$grepaiLogsDir = Join-Path `$env:LOCALAPPDATA 'grepai\logs'", $start)
        $start | Should -BeGreaterThan 0
        $end | Should -BeGreaterThan $start -Because 'the bootstrap block must be bounded by the spawn block'
    ...
    It 'cannot abort the launch: Get-Command guard, try/catch, and no exit or throw' {
        ...
        $block | Should -Match 'if \(Get-Command Invoke-McpBootstrapForRepo -ErrorAction SilentlyContinue\) \{'
    ...
    It 'calls a function the module really exports (no silent rename on either side)' {
        ...
        $defined | Should -Contain 'Invoke-McpBootstrapForRepo'
        # Exactly one aggregate. (Invoke-McpBootstrapCommand is the per-command
        # runner, not a second aggregate, so the match is exact.)
        @($defined | Where-Object { $_ -eq 'Invoke-McpBootstrapForRepo' }).Count | Should -Be 1
    }
    ...
    It 'keeps the module contract the launcher depends on: never throws, degrades to a row' {
        ...
        $text | Should -Match "New-McpBootstrapRow -Mcp \`$step\.Mcp -Status 'skipped'"
